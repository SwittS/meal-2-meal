...
function update(req, res) {
    ...
    console.log(id);
    db.Ingredient.findByIdAndUpdate(id, updateData, {
        new: true
    }, function(err, savedUpdatedIngredient) {
        if (err) {
...
function destroy(req, res) {
    db.Ingredient.findOneAndRemove({
        _id: req.params.ingredientsId
    }, function(err, ingredientToDelete) {
        if (err) {
